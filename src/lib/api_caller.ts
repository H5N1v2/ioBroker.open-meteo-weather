...
export interface WeatherConfig {
	...
	/** Ob 15 Minutenvorhersage aktiviert ist */
	forecast15Enabled: boolean;
	/** Anzahl Datensätze für 15 minuten vorhersage */
	forecast15: number;
...
export async function fetchAllWeatherData(config: WeatherConfig, logger?: ioBroker.Logger): Promise<any> {
	...
	const currentparam_keys =
		'temperature_2m,relative_humidity_2m,pressure_msl,apparent_temperature,precipitation,rain,showers,snowfall,weather_code,cloud_cover,wind_speed_10m,wind_direction_10m,wind_gusts_10m,is_day';

	const dailyparam_keys =
		'relative_humidity_2m_mean,weather_code,temperature_2m_max,temperature_2m_min,pressure_msl_mean,sunrise,sunshine_duration,sunset,uv_index_max,precipitation_sum,rain_sum,showers_sum,snowfall_sum,precipitation_probability_max,et0_fao_evapotranspiration_sum,cloud_cover_max,wind_speed_10m_max,wind_direction_10m_dominant,wind_gusts_10m_max,dew_point_2m_mean';

	// URL mit unitParams erweitern
	const weatherUrl = `https://api.open-meteo.com/v1/forecast?latitude=${config.latitude}&longitude=${config.longitude}&current=${currentparam_keys}&daily=${dailyparam_keys}${fHoursParam_keys}${f15Param_keys}&timezone=${tz}&forecast_days=${config.forecastDays}${fHoursParam}${f15Param}${unitParams}`;
