...
async function fetchAllWeatherData(config, logger) {
  ...
  const currentparam_keys = "temperature_2m,relative_humidity_2m,pressure_msl,apparent_temperature,precipitation,rain,showers,snowfall,weather_code,cloud_cover,wind_speed_10m,wind_direction_10m,wind_gusts_10m,is_day";
  ...
  if (config.airQualityEnabled) {
    ...
    try {
      const resA = await import_axios.default.get(airUrl, axiosConfig);
      if (logger) {
